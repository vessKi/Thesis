...
# build my own model and make it track pens  
# Load a model
#model = YOLO('yolov8n.pt')  # Load an official Detect model
#model = YOLO('yolov8n-seg.pt')  # Load an official Segment model
#model = YOLO('yolov8n-pose.pt')  # Load an official Pose model
model = YOLO('C:/Users/kilia/Desktop/COOP/modelbig.pt')
...
def yolo_inference_thread(cap):
    while True:
        ret, frame = cap.read()
        if not ret:
            break  # Stop the loop if we, by which i mean myself and my crazy self can't get frames
        ...
        predictions_queue.put(frame_with_predictions)
...
inference_thread = threading.Thread(target=yolo_inference_thread, args=(cap,), daemon=True)
...
while True:
    ...
    if not predictions_queue.empty():
       ...
       frame_with_predictions = predictions_queue.get()
       cv2.imshow('YOLOv8 Detection', frame_with_predictions)
       
    # # frame_tensor = transforms.ToTensor()(frame).unsqueeze_(0)  # Add batch dimension
    # # with torch.no_grad():
    ...
    # frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    # # Perform inference (the model expects RGB images)
    # results = model.track(frame_rgb, persist=True)
    # # Render predictions on the original BGR frame for display
    # frame_with_predictions = results[0].plot()
    # # Display the frame with bounding boxes and labels
    # cv2.imshow('YOLOv8 Detection', frame_with_predictions)
    # # Display the annotated frame (with box)
